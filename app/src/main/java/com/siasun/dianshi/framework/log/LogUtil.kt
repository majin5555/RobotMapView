...
object LogUtil {
    ...
    fun init(
        ...
        val envInfo = """
            *******************************
            ${DeviceInfoUtils.phoneManufacturer}_${DeviceInfoUtils.phoneModel}_${metrics.widthPixels}x${metrics.heightPixels}_${metrics.densityDpi}_${Build.VERSION.SDK_INT}_V${AppUtils.getAppVersionName()}(${AppUtils.getAppVersionCode()})
            *******************************
        """.trimIndent()
    ...
    private fun prepareLog(
        ...
        throwable?.let {
            ...
            // 按照日志级别打印，线上版本在Info及以上级别会打印到日志文件中
            when (priority) {
