...
class CreateMap2DActivity :
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        mBinding.tvCreate.onClick {
            ...
            mBinding.mapView.setWorkMode(CreateMapView2D.MODE_CREATE_MAP2D)
    ...
    private fun navHeartbeatState(it: ByteArray) {
        //扫描新环境和扩展环境 都不锁屏
//            LogUtils.d("导航心跳 MSG_NAVI_STATE it[0]---${it[0]}")
//        LogUtils.d("导航心跳 MSG_NAVI_STATE it[1]---${it[1]}")
//            LogUtils.d("iParams[2].toInt()---${it[2]}")
        //导航当前状态
        when (it[0].toInt()) {
            //定位
            1 -> {
                if (mBinding.mapView.isMapping) {
                    LogUtil.i(
                        "此时导航从其他模式切换到定位，说明导航已经建图、优化、保存完成",
                        null,
                        TAG_NAV
                    )
...
//            }).create().show()
    ...
    private fun showSavaMapDialog() {
        CommonWarnDialog.Builder(this).setMsg("保存地图")
            .setOnCommonWarnDialogListener(object :
                CommonWarnDialog.Builder.CommonWarnDialogListener {
                override fun confirm() {
                    //开始保存地图
                    MainController.saveEnvironment(1, mapId = 1)
                ...
                override fun discard() {
                    mBinding.mapView.isMapping = false
                    MainController.saveEnvironment(2, mapId = 1)
                    SEND_NAVI_HEART = false
