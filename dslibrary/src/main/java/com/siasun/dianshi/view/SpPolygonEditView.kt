...
class SpPolygonEditView(context: Context?, parent: WeakReference<MapView>) :
    ...
    var list: MutableList<SpArea> = mutableListOf()
    ...
    fun setWorkMode(mode: MapView.WorkMode) {
        ...
        // 如果退出编辑模式，清空选中状态
        if (mode != MapView.WorkMode.MODE_SP_AREA_ADD && mode != MapView.WorkMode.MODE_SP_AREA_EDIT) {
            selectedArea = null
...
//        newArea.m_VertexPnt.add(bottomLeft)
     ...
     * 查找并返回点击位置附近的顶点索引
     */
    private fun findNearbyVertexIndex(area: SpArea, screenX: Float, screenY: Float): Int {
     ...
     * 查找并返回点击位置附近的边索引
     */
    private fun findNearbyEdgeIndex(area: SpArea, screenX: Float, screenY: Float): Int {
     ...
     * 在边上添加新顶点
     */
    private fun addVertexOnEdge(area: SpArea, edgeIndex: Int) {
        val points = area.m_VertexPnt
     ...
     * 删除指定的边
     */
    private fun removeEdge(area: SpArea, edgeIndex: Int) {
        val points = area.m_VertexPnt
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        if ((currentWorkMode != MapView.WorkMode.MODE_SP_AREA_ADD && currentWorkMode != MapView.WorkMode.MODE_SP_AREA_EDIT) || selectedArea == null) {
            return false
        }
     ...
     * 绘制单个不规则图形区域
     */
    private fun drawPolygon(canvas: Canvas, area: SpArea, isSelected: Boolean) {
        val points = area.m_VertexPnt
        ...
        when (area.routeType) {
            ...
            AreaType.AREA_CARGO_LOCATION -> areaPaint.setColor(-0x6f00ff01)// 洋红 货位区
            AreaType.AREA_MEMORY_BLOCKING -> areaPaint.setColor(-0x6f005b00)//橙色 记忆屏蔽区
            AreaType.AREA_SONAR_SHIELDING -> areaPaint.setColor(-0x6f800000)////栗色 声呐屏蔽区
            AreaType.AREA_RESTRICTED -> areaPaint.setColor(-0x6f32a3a4)//棕色 限制区
        }
        ...
        if ((currentWorkMode == MapView.WorkMode.MODE_SP_AREA_ADD || currentWorkMode == MapView.WorkMode.MODE_SP_AREA_EDIT) && isSelected) {
            // 绘制所有顶点
            for (i in points.indices) {
                val screenPoint =
