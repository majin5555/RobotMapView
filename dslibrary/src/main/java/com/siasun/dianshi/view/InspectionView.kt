...
class InspectionView(context: Context?, var parent: WeakReference<MapView>) :
    SlamWareBaseView<MapView>(context, parent) {
    private var radius = 10f

    // 避让点 - 使用同步列表确保线程安全
    private val mInspection = ArrayList<Inspection>()

    // 用于点击检测的避让点屏幕坐标映射
    private val stationScreenPositions = mutableMapOf<Inspection, Pair<Float, Float>>()
    ...
    interface OnStationClickListener {
        fun onStationClick(station: Inspection)
    }

    // 删除事件回调接口
    interface OnStationDeleteListener {
        fun onStationDelete(station: Inspection)
    }
            ...
            mInspection.forEach { station ->
                station.point.let {
                    ...
                    when (currentWorkMode) {
                        WorkMode.MODE_INSPECTION_STATION_EDIT -> {
    ...
    fun setInspectionViewStations(list: MutableList<Inspection>?) {
        ...
        list?.let {
            // 预分配容量，避免频繁扩容
            mInspection.ensureCapacity(it.size)
            mInspection.addAll(it)
            postInvalidate()
        }
    }
